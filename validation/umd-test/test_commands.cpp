...
TEST_F(Command, NNAndCopyCommandQueueCreationAndMultipleCommandsSubmission) {
    ...
    scopedQueueVec.push_back(zeScope::commandQueueCreate(zeContext, zeDevice, cmdQueueDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    auto copyQueue = scopedQueueVec.back().get();

    cmdListDesc.commandQueueGroupOrdinal = copyGrpOrdinal;
    scopedListVec.push_back(zeScope::commandListCreate(zeContext, zeDevice, cmdListDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    auto listCopy = scopedListVec.back().get();
    ...
    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(list, ts, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(listCopy, ts, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);
    ...
    *copyInputHost = 0xdeadbeef;
    ...
    auto mem3 = AllocDeviceMemory(size);
    void *memOutputDev = mem3.get();
    ASSERT_NE(memOutputDev, nullptr) << "Failed to allocate device memory";
    ASSERT_EQ(
        zeCommandListAppendMemoryCopy(list, memOutputDev, memInputHost, size, nullptr, 0, nullptr),
        ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListAppendMemoryCopy(listCopy,
                                            memOutputDev,
                                            memInputHost,
                                            size,
                                            nullptr,
    ...
    auto mem4 = AllocHostMemory(size);
    void *memOutputHost = mem4.get();
    ASSERT_NE(memOutputHost, nullptr) << "Failed to allocate host memory";
    uint64_t *copyOutputHost = static_cast<uint64_t *>(memOutputHost);
    ASSERT_EQ(
        zeCommandListAppendMemoryCopy(list, memOutputHost, memOutputDev, size, nullptr, 0, nullptr),
        ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListAppendMemoryCopy(listCopy,
                                            memOutputHost,
                                            memOutputDev,
    ...
    auto mem5 = AllocSharedMemory(size);
    uint64_t *ts2 = static_cast<uint64_t *>(mem5.get());
    ...
    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(list, ts2, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(listCopy, ts2, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandListClose(list), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListClose(listCopy), ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandQueueExecuteCommandLists(queue, 1, &list, nullptr), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandQueueExecuteCommandLists(copyQueue, 1, &listCopy, nullptr),
              ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandQueueSynchronize(queue, syncTimeout), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandQueueSynchronize(copyQueue, syncTimeout), ZE_RESULT_SUCCESS);
    ...
    EXPECT_NE(*ts, *ts2) << "Timestamp values should be different";
}

TEST_F(Command, MultipleCommandQueueCreationAndCommandsSubmissionToBothQueues) {
    ze_result_t ret;
    cmdQueueDesc.ordinal = computeGrpOrdinal;
    scopedQueueVec.push_back(zeScope::commandQueueCreate(zeContext, zeDevice, cmdQueueDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    ...
    scopedListVec.push_back(zeScope::commandListCreate(zeContext, zeDevice, cmdListDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    auto copyList = scopedListVec.back().get();
    ...
    auto mem1 = AllocHostMemory(size);
    uint64_t *memInputHost = static_cast<uint64_t *>(mem1.get());
    *memInputHost = 0xdeadbeef;
    auto mem2 = AllocDeviceMemory(size);
    uint64_t *memOutputDev = static_cast<uint64_t *>(mem2.get());
    auto mem3 = AllocHostMemory(size);
    uint64_t *memOutputHost = static_cast<uint64_t *>(mem3.get());
    ASSERT_EQ(
        zeCommandListAppendMemoryCopy(list, memOutputDev, memInputHost, size, nullptr, 0, nullptr),
        ZE_RESULT_SUCCESS);
    ASSERT_EQ(
    ...
    auto mem4 = AllocHostMemory(size);
    uint64_t *memInputHost2 = static_cast<uint64_t *>(mem4.get());
    *memInputHost2 = 0xabcdabcd;
    auto mem5 = AllocDeviceMemory(size);
    uint64_t *memOutputDev2 = static_cast<uint64_t *>(mem5.get());
    auto mem6 = AllocHostMemory(size);
    uint64_t *memOutputHost2 = static_cast<uint64_t *>(mem6.get());
    ASSERT_EQ(zeCommandListAppendMemoryCopy(copyList,
                                            memOutputDev2,
                                            memInputHost2,
                                            size,
                                            nullptr,
    ...
    auto mem7 = AllocSharedMemory(size);
    uint64_t *ts2 = static_cast<uint64_t *>(mem7.get());
    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(copyList, ts2, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandListClose(list), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListClose(copyList), ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandQueueExecuteCommandLists(queue, 1, &list, nullptr), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandQueueSynchronize(queue, syncTimeout), ZE_RESULT_SUCCESS);
    EXPECT_NE(*ts, 0llu) << "Timestamp should be different from 0";
    EXPECT_EQ(*memInputHost, *memOutputHost) << "Copy failed, numbers should be equal";

    ASSERT_EQ(zeCommandQueueExecuteCommandLists(copyQueue, 1, &copyList, nullptr),
              ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandQueueSynchronize(copyQueue, syncTimeout), ZE_RESULT_SUCCESS);
    EXPECT_EQ(*memInputHost2, *memOutputHost2) << "Copy failed, numbers should be equal";
    EXPECT_NE(*ts2, 0llu) << "Timestamp should be different from 0";
    EXPECT_NE(*ts, *ts2) << "Timestamp values should be different";
}
...
TEST_F(CommandTimestamp, CopyQueueTimestampSubmission) {
    ze_result_t ret;
    cmdQueueDesc.ordinal = copyGrpOrdinal;
    scopedQueueVec.push_back(zeScope::commandQueueCreate(zeContext, zeDevice, cmdQueueDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
...
TEST_F(CommandTimestamp, TwoCommandQueusWithDifferentGroupExecuteTimestampAndSynchronize) {
    ze_result_t ret;
    scopedQueueVec.push_back(zeScope::commandQueueCreate(zeContext, zeDevice, cmdQueueDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    auto queueNN = scopedQueueVec.back().get();

    cmdQueueDesc.ordinal = copyGrpOrdinal;
    scopedQueueVec.push_back(zeScope::commandQueueCreate(zeContext, zeDevice, cmdQueueDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    auto queueCopy = scopedQueueVec.back().get();

    cmdListDesc.commandQueueGroupOrdinal = computeGrpOrdinal;
    scopedListVec.push_back(zeScope::commandListCreate(zeContext, zeDevice, cmdListDesc, ret));
    ASSERT_EQ(ret, ZE_RESULT_SUCCESS);
    auto listNN = scopedListVec.back().get();
    ...
    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(listNN, ts, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListClose(listNN), ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandListAppendWriteGlobalTimestamp(listCopy, ts + 1, nullptr, 0, nullptr),
              ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandListClose(listCopy), ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandQueueExecuteCommandLists(queueNN, 1, &listNN, nullptr), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandQueueExecuteCommandLists(queueCopy, 1, &listCopy, nullptr),
              ZE_RESULT_SUCCESS);

    ASSERT_EQ(zeCommandQueueSynchronize(queueNN, syncTimeout), ZE_RESULT_SUCCESS);
    ASSERT_EQ(zeCommandQueueSynchronize(queueCopy, syncTimeout), ZE_RESULT_SUCCESS);
