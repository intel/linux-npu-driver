...
inline SharedPtr<ze_graph_handle_t> graphCreate3(graph_dditable_ext_t *ddi,
                                                 ze_context_handle_t ctx,
                                                 ze_device_handle_t dev,
                                                 const ze_graph_desc_2_t &desc,
                                                 ze_graph_build_log_handle_t *pBuildLogHandle,
                                                 ze_result_t &ret) {
    ze_graph_handle_t handle = nullptr;
    ...
    ret = ddi->pfnCreate3(ctx, dev, &desc, &handle, pBuildLogHandle);
    if (ret != ZE_RESULT_SUCCESS)
        return nullptr;
