...
void SHA1Transform(uint32_t state[5], const uint8_t buffer[SHA1_BLOCK_LENGTH]) {
    ...
    (void)memcpy(block, buffer, SHA1_BLOCK_LENGTH); // NOLINT
...
void SHA1Update(SHA1_CTX *context, const uint8_t *data, size_t len) {
    ...
    if ((j + len) > 63) {
        (void)memcpy(&context->buffer[j], data, (i = 64 - j)); // NOLINT
...
#pragma GCC diagnostic pop
    ...
    (void)memcpy(&context->buffer[j], &data[i], len - i); // NOLINT
}
...
void SHA1Pad(SHA1_CTX *context) {
    ...
    for (i = 0; i < 8; i++) {
        finalcount[i] =
            (uint8_t)((context->count >> ((7 - (i & 7)) * 8)) & 255); /* Endian independent */
    }
...
#ifdef ANDROID
    memset_explicit(context, 0, sizeof(*context));
#else
